...
RSpec.describe User, type: :model do
  ...
  it 'requires a name' do
    user = User.new(name:'', email:'kobe@lakers.com', password:'password', password_confirmation:'password')
    user.valid?
    expect(user.errors[:name].any?).to eq(true)
  end
  it 'requires an email' do
    user = User.new(name:'kobe', email:'', password:'password', password_confirmation:'password')
    user.valid?
  ...
  it 'accepts properly formatted email' do
    ...
    emails.each do |email|
      user = User.new(name:'kobe', email: email, password:'password', password_confirmation:'password')
      user.valid?
  ...
  it 'requires a unique, case insensitive email address' do
    ...
    user2 = User.new(name:'kobe2', email:user1.email.upcase, password:'password', password_confirmation:'password')
    user2.valid?
    expect(user2.errors[:email].first).to eq("has already been taken")
  end
  it 'requires a password' do
    user = User.new(name:'kobe', email:'kobe@lakers.com', password:'', password_confirmation:'password')
    user.valid?
    expect(user.errors[:password].any?).to eq(true)
  end
  it 'requires the password to match the password confirmation' do
    user = User.new(name:'kobe', email:'kobe@lakers.com', password:'password', password_confirmation:'not password')
    user.valid?
